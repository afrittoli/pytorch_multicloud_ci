...
\begin{grayframe}
  ...
  \begin{itemize}
    ...
    \item All PRs
  \end{itemize}
  % TODO Include requirements-per-role on the side
\end{grayframe}
\note[itemize]{
...
\begin{grayframe}
  ...
  \end{itemize}
  % TODO On the left side diagram that shows example of
...
\begin{grayframe}
  ...
  % TODO Diagram diagrams-Process.png
  ~\\
  \begin{itemize}
...
\begin{grayframe}
  ...
  \end{itemize}
  % TODO Probably one Hud and one GHARTS screenshot for this
\end{grayframe}
\note[itemize]{
...
\begin{grayframe}
  ...
  % TODO pytorch-multi-cloud-infra-Overview.png
\end{grayframe}
...
\begin{grayframe}
  ...
  \begin{itemize}
    ...
    \item \textbf{Infrastructure as Code:} Reusable terraform modules
  \end{itemize}
  % TODO Use the prototype architecture picture here
\end{grayframe}
\note[itemize]{
...
\begin{grayframe}
  \frametitle{Vendor-Managed GitHub Runners}
    % TODO reminder of original challenges, specific for this?
  \textbf{GitHub Actions Runner Token Service (GHARTS):}
  \begin{itemize}
    \item Secure, self-service API to register self-hosted runners
    \item No long-lived credentials, on demand short-lived tokens
    \item Enforcement of runner metadata (label, group) and quotas
    \item Central administration, bulk operations
    \item Audit logging for all runner operations
    \item MVP being deployed, IBM to trial service
  \end{itemize}
  % TODO Use the GHARTS dashboard screenshots
\end{grayframe}
\note[itemize]{
...
\begin{grayframe}
  ...
  \begin{itemize}
    ...
    \item Vendor requests a JIT config from GHARTS using the JWT % TODO JWT
...
\begin{grayframe}
  ...
  \begin{itemize}
    \item a
    \item b
    % TODO Discuss GitHub dependent workflow
    % TODO Considering alternatives OpenTelemetry / CDEvents
  \end{itemize}
\end{grayframe}
\note[itemize]{
...
\begin{grayframe}
  ...
  \textbf{Achievements:}
  \begin{itemize}
    % TODO: describe vendors onboarded via each interface
    % Public Cloud, Runner Pools, Vendor Systems
...
\begin{grayframe}
  ...
  % TODO: Ongoing initiatives IBM on GHARTS
  % Don't have onboarding expeience from Vendor Systems
  % RedHat for Vendor System visibility
  % Vendors happy to share their experience and collaborate
  \begin{itemize}
...
\begin{grayframe}
  ...
  % TODO Key slide, maybe merged with the next one
  % I want to highlight key principles that can be
  % reused by other project
  % How to protect everyone's interest in a fair way
  % Remove attrition and avoid stale
  % Engage vendors, let them help build the solution
  % Empower infra maintainers, care about scale
  \textbf{What Worked:}
  \begin{itemize}
...
\begin{grayframe}
  ...
  \textbf{What's Next:}
  % TODO ongoing activities in short
  % Most what's next has already been presented
  % Maybe add more about monitoring?
  \begin{itemize}
...
\begin{grayframe}
  ...
  % TODO this sounds like another repeat of what's said already
  % Probably going away, may move the slide above down here
  \begin{itemize}
